...
# Detect Exact Date of Breakpoint Bfast01 Daily Time Series
d <- breakdates(bf1)

# Convert Orginal Date Format to Exact Date of Daily Time Series
d_converted <- date_decimal(d)

# Detect Exact Date of Breakpoint Bfast01 Monthly Time Series
d2 <- breakdates(bf2)

# Convert Orginal Date Format to Exact Date of Daily Time Series
d_converted1 <- date_decimal(d2)

# Save to Disk Daily Time Series
d3 <- date_decimal(d)
d3_cap <- capture.output(d3)
tex <- gsub(pattern = "\\.csv$", "", i)
prip <- paste("BreakPoint_Date_Daily", tex, ".txt")
zar <- cat(d3_cap, file=prip, sep="\n", append=FALSE)

# Save to Disk Monthly Time Series
d4 <- date_decimal(d2)
d4_cap <- capture.output(d4)
tex1 <- gsub(pattern = "\\.csv$", "", i)
prip1 <- paste("BreakPoint_Date_Monthly", tex1, ".txt")
zar1 <- cat(d4_cap, file=prip1, sep="\n", append=FALSE)
}
