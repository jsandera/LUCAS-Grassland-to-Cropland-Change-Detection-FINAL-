...
transform_df <- function(x=A){
  ...
  b <- cbind(a, subset(x, select=c("point_id", "EVI", "NDVI", "NDWI"))) # select columns for new data frame
  ...
  i <- subset(h, select=c("point_id", "Time", "EVI", "NDVI", "NDWI"))
  return(i)
}
...
# Scale Function
sc <- preProcess(as.data.frame(B[,2:5]), method=c("range"))
