...
transform_df <- function(x=A){
  ...
  b <- cbind(a, subset(x, select=c("point_id", "EVI", "NDVI", "NDMI", "NDWI"))) # select columns for new data frame
  ...
  i <- subset(h, select=c("point_id", "Time", "EVI", "NDVI", "NDMI","NDWI"))
  return(i)
}
...
# Scale Function
sc <- preProcess(as.data.frame(B[,2:6]), method=c("range"))
